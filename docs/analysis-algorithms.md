...
대시보드 **분석 탭**에 남아 있는 6개 위젯이 각각 어떤 알고리즘으로 결과를 만드는지 정리한 문서다.
(2026-07 기준. 위젯 정리 후 상태 — 날짜별/시간대별/요일별/말머리/키워드카드/감성/연관어는 제거됨.)
...
3. [LLM 심층 분석](#2-llm-심층-분석-keyword_report)
...
## 2. LLM 심층 분석 (`keyword_report`)
`analysis/llm_report.py:keyword_report` · `POST /api/analysis/llm_report`

**목적**: 키워드가 들어간 글·댓글을 LLM으로 읽어 **무슨 얘기·평가·쟁점**이 오갔는지 정리하고, 각 항목에 **근거 글 번호**를 붙인다. (빈도 분석이 "어떤 단어"를 알려준다면 이건 "무슨 내용")

**알고리즘 (map-reduce)**:
1. **문서 수집** (`_build_docs`): 키워드를 하나라도 포함(OR)하는 비성인 글을 로드 → **추천순 정렬** → 상위 `max_posts`(기본 60)개만 채택. 잘리면 `truncated=true`.
2. **문서 블록화**: 글마다 `[글 #번호] 제목(추천n) / 본문(최대 800자) / 댓글(최대 15개, 각 180자)` 형태로 구성.
3. **배치**: 문자 예산(14,000자) 기준으로 블록을 묶음.
   - 묶음 1개 → **단일 호출**(`_SINGLE_SYS`)
   - 여러 개 → **map**(각 묶음 요약) 후 **reduce**(부분 결과 JSON들을 하나로 종합, 근거 글번호는 합집합 보존)
4. **출력 JSON**: `overview`(요약), `mood`(분위기), `themes`(주요 주제), `positives`/`negatives`(긍·부정 평가), `issues`(쟁점), `quotes`(대표 인용). 각 항목의 `post_nos` → 원본 글 링크로 변환.
5. **캐시**: `(키워드, 모델, source, 필터, 대상 글번호들)`의 SHA1 해시를 키로 DB(`llm_reports`)에 저장. 같은 요청은 즉시 반환, `refresh=true`로 무시.

**한계**: 전체가 아니라 **추천순 상위 60글**만 실제 분석 대상. `post_count`(매칭 전체) vs `analyzed_posts`(분석된 수)로 표시.
