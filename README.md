...
| 📈 참여도 추이 | 날짜별 조회·추천·댓글 합계/평균으로 반응의 크기 변화를 본다 |
| 키워드 빈도 | 형태소 분석으로 제목/본문/댓글에서 자주 나온 단어를 뽑는다 |
| ⭐ 특징어(TF-IDF) | 흔한 단어를 눌러 **이 갤/기간에 유독 튀는 단어**를 상위로 뽑는다(bigram 포함) |
| 🔥 이슈 버스트 | 전일 대비 **급상승·신규 등장 키워드**로 "오늘의 떡밥"을 잡는다 |
| ☁️ 워드클라우드 | 키워드 빈도를 시각적으로 보여준다 (단어 클릭 → 연관어 분석) |
| 🔗 연관어 분석 | 특정 키워드가 나오는 글에서 **함께 등장하는 단어**를 PMI로 찾는다(흔한 단어 독식 방지) |
| 💬 감성/여론 분석 | 글·댓글의 긍정/부정/중립 분포로 여론 분위기를 가늠한다(슬랭·강조·이모티콘 반영) |
| 🌡️ 감성 추이 | 날짜별 평균 감성점수로 여론 분위기 변화를 추적한다 |
| 🧠 LLM 심층 분석 | 특정 키워드가 든 글·댓글을 LLM으로 읽어 **무슨 내용·평가·쟁점**이 있는지 서술로 정리한다(단어 나열을 넘어선 맥락 요약) |
...
| 분석 | `pandas` (집계), `kiwipiepy` (한국어 형태소 분석, Java 불필요), TF-IDF·PMI, 사전 기반 감성 |
| LLM | Claude Sonnet via **OpenRouter**(`requests`) 또는 Anthropic(`anthropic`) — 키워드 심층 분석(글·댓글 map-reduce 요약), 결과 DB 캐시 |
...
/api/analysis/timeseries?kind=date|hour|weekday|sentiment|engagement  활동/감성/참여도 추이
/api/analysis/heatmap                            요일×시간 히트맵
/api/analysis/keywords?method=count|salient&source=all&top_n=120  키워드 빈도 / 특징어(TF-IDF)
/api/analysis/bursts?date=YYYY-MM-DD&min_count=2  이슈 버스트(급상승·신규 키워드)
/api/analysis/related?word=출력&source=all        연관어(PMI) 분석
/api/analysis/sentiment?source=comment           감성 분포
POST /api/analysis/llm_report {"q":"에덴"}         LLM 심층 분석(글·댓글 요약)
/api/analysis/llm_status                          LLM 사용 가능 여부(키 설정)
예) /api/stats/overview?q=에덴  → "에덴" 포함 글만 통계
```

**🧠 LLM 심층 분석 설정** — 키워드가 든 글·댓글을 LLM으로 읽어 내용·평가·쟁점을 정리합니다.
...
./run.sh                  # .env 자동 로드 → 분석 탭 → 🧠 LLM 심층 분석 카드
```
...
# 방법 B) Anthropic 직접 (anthropic SDK 사용)
...
결과는 `dcinside.db`의 `llm_reports` 테이블에 캐시되어 같은 키워드 재조회는 무료·즉시입니다
(`새로 분석` 체크 시 재생성). 큰 키워드는 map-reduce로 여러 번 나눠 호출하며, 응답 JSON이
깨지면 1회 자동 복구를 시도합니다.
